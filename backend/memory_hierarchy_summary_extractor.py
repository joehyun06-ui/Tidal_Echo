"""Strict provider-agnostic hierarchy summary extractor for Phase 4D-B6.

The provider receives one already-proved Topic or Canonical-State target as
UNTRUSTED DATA and may return only source-bound clauses.  It cannot choose the
node, projection digest, authority, summary contract, sensitivity, or any Memory
state.  Server-side validation requires complete Atomic coverage and re-applies
MemoryPolicy to every returned clause.

Generated text is routing/compression material only and must never be treated as
Memory truth without returning to the supporting Atomic Memory rows.
"""
...
EXTRACTOR_CONTRACT_VERSION: Final = "memory-hierarchy-summary-extractor-v1"
EXTRACTOR_SESSION_ID: Final = "memory-hierarchy-summary-extractor-v1"
...
_ERROR_CATEGORIES: Final = frozenset({
    ...
    "memory_hierarchy_summary_extractor_error",
...
{"version":"memory-hierarchy-summary-extractor-v1","clauses":[{"memory_keys":["memory_key_1"],"text":"Concise supported statement."}]}
You may output only existing memory_key strings from the supplied records and clause text supported by those exact records. Never output node keys, projection digests, authority fields, sensitivity, confidence, labels, metadata, explanations, citations outside memory_keys, or new facts.
Every supplied memory_key must be covered by at least one clause. A clause may cite multiple records only when all of them support that statement. Do not invent causal links, dates, identities, motivations, or conclusions not present in the records.
...
For target_type=topic, summarize the durable subject/workstream. episode_groups are organization hints only; they are not facts and must not be described unless the underlying records support the wording.
Keep clauses concise. Generated text is routing/compression material only, not Memory truth.
"""


class MemoryHierarchySummaryExtractorError(ValueError):
    __slots__ = ("category",)
    ...
    def __init__(self, category: object):
        safe = (
            ...
            else "memory_hierarchy_summary_extractor_error"
        )
        self.category = safe
        super().__init__(safe)
    ...
    def __str__(self) -> str:
        try:
            ...
            return "memory_hierarchy_summary_extractor_error"

    def __repr__(self) -> str:
        return f"MemoryHierarchySummaryExtractorError({str(self)!r})"


def _raise(category: str) -> None:
    raise MemoryHierarchySummaryExtractorError(category)
...
def _parse_model_output(
    ...
    try:
        return summary.validate_summary_clauses_v1(target, tuple(clauses))
    except summary.MemoryHierarchySummaryError:
        _raise("extractor_invalid_output")


async def extract_node_summary_v1(
    ...
    *,
    ...
    """Run one bounded source-bound summary extraction for a proved node."""
    ...
    try:
        target = summary.prepare_summary_target_v1(atomics, plan, node_key)
    except summary.MemoryHierarchySummaryError:
        _raise("summary_target_invalid")
    ...
    context = {
        ...
        "memory_hierarchy_summary_contract": summary.SUMMARY_CONTRACT_VERSION,
