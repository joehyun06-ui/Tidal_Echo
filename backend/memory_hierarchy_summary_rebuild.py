"""Unwired summary-cache rebuild composition for Phase 4D-B6C.

One invocation proves the complete authoritative active Atomic Memory snapshot,
loads the current content-free hierarchy sidecar, reconstructs and re-proves the
hierarchy plan, then materializes only missing/stale Topic and Canonical-State
summaries in the separate derived-text cache.

Clean current summaries never call the provider.  Per-node provider extraction
failure is optional and resumable: it increments a bounded failure count while
leaving Atomic Memory, hierarchy state, and other valid cache entries untouched.
Source, hierarchy, or cache integrity failure is fatal and data-free.
"""
...
from backend import (
    ...
    memory_hierarchy_summary_extractor as summary_extractor,
    memory_hierarchy_summary_store as summary_store,
)


SUMMARY_REBUILD_CONTRACT_VERSION: Final = "memory-hierarchy-summary-rebuild-v1"
...
class MemoryHierarchySummaryRebuildError(RuntimeError):
    """Stable, data-free fatal summary-rebuild failure."""
    ...
    def __repr__(self) -> str:
        return f"MemoryHierarchySummaryRebuildError({str(self)!r})"


def _raise(category: str) -> None:
    raise MemoryHierarchySummaryRebuildError(category)


@dataclass(frozen=True, slots=True, repr=False)
class HierarchySummaryRebuildReceiptV1:
    ...
    def __repr__(self) -> str:
        return (
            "<HierarchySummaryRebuildReceiptV1 "
...
def _plan_from_sidecar_snapshot(
    ...
    try:
        ...
        return plan
    except MemoryHierarchySummaryRebuildError:
        raise
...
def _summary_targets(
    ...
    targets = tuple(
        ...
        if node.node_type in {"topic", "canonical_state"}
    )
...
async def rebuild_current_hierarchy_summaries_v1(
    reader: object,
    hierarchy_sidecar_path: object,
    summary_store_path: object,
    generation_callable,
    *,
    ...
    """Refresh only current cache misses/stale Topic/State summary revisions."""
    ...
    for node in targets:
        ...
        try:
            derived = await summary_extractor.extract_node_summary_v1(
                counted_generation,
                authority_snapshot.atomics,
                plan,
                node.node_key,
            ...
            raise
        except summary_extractor.MemoryHierarchySummaryExtractorError:
            failed += 1
            continue
        except Exception:
            failed += 1
    ...
    return HierarchySummaryRebuildReceiptV1(
